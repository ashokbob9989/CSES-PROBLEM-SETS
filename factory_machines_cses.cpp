...
int32_t main()
{
    ...
    ll low=0,high=inf;  // actuallt its a predicate logic algo... f f f f t t t t.. we need to find the first "t".. we use binary search
    while(low<=high)
    {
        ll mid=(low+high)/2;
        ll here_prod_made=0; 
        ...
        for(ll i=0;i<n;i++)
        {
            here_prod_made+=(mid/v[i]);
            if(here_prod_made>=t)
            break;  // over_flow conditions to be avoided
        }
        if(here_prod_made>=t)
        {
            min_time=mid;
