...
bool dfs(ll ver, ll par, ll col)
{
    ...
    for(auto child : adj[ver])
    {
        ...
        if(color[child]==0)
        if(!dfs(child,ver,color[ver]^3)) // for 1st team we mark as 1... and second team as 2 ... so for if a node goes to 1 we xor it with 3 thatis 0001 ^ 0011 it gives 0010(which is 2) so another node is marked as 2nd team 
        return false;
...
bool all_colored()
{
    ...
    {
        if(!vis[i])
        if(!dfs(i,-1,1)) // vertex, its-parent, color type.
        return false;
    }
    return true;
}
