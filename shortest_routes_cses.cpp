...
ll n, m;
vvpll g(N);
vll dist(N,inf); // initially all ae at distance == infinity
 
void dijkstra()
{
 
    priority_queue<pll,vpll,greater<pll>> pq; // stores weight,node ..in max heap manner

    dist[1] = 0; // distance of first node to first node is '0'
    pq.push({0,1}); // (wt/dist) , node
    while(!pq.empty())
    {
        ll wt = pq.top().first; //dist/wt 
        ll ver = pq.top().second; // its vertex
        pq.pop();
        ...
        {
            ll here_ver = child.first; // note:: pq stores in pair like (wt, vertex)
            //but here vector of pair stoes (node, wt).
            ll here_wt = child.second;

            if(dist[here_ver] <= here_wt+wt)
                continue;

            else
            {
                dist[here_ver] = here_wt + wt;
                pq.push({dist[here_ver], here_ver});
            }
...
int32_t main()
{
    ...
    for(auto i = 0; i < m; ++i)
    {
        ll x, y, wt;
        cin >> x >> y >> wt;
        g[x].pb({y,wt}); // x is connected to y with weight==wt
    }

    dijkstra();
    for(ll i = 1; i <=n; ++i )
    {
        cout << dist[i] << " "; // shortest distance from each node to another.
    }
