...
int32_t main()
{
    ...
    ll maxii=*max_element(allin(v));
    ll low=maxii;  // we start from maxii element in array to inf .... f f f f f ...t t t t .. we find first true wherewe are getting our partitins satisfes. 
    ll high=inf;
    ll ans=inf;
    while(low<=high)
    {
        ...
        ll blocks=1; // we have atleast one block;
        ll calci=0;

        for(ll i=0;i<n;i++)
        {
            if(calci+v[i]> mid) // it says we exceeded range for a block so we need to start new block
            {
                blocks++; // we start new block
                calci=0;  // for next block sum
            }
            calci+=v[i]; // else w e will add v[i] to calci
        }
        if(blocks>k)// we exceeded no .of blocks ..so this mid  is not valid
        low=mid+1;
